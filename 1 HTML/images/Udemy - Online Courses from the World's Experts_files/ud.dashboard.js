define(['jquery-widget-init', 'handlebars.helpers', 'handlebars-templates', '../common/constants',
    'ud.raven', 'jquery.rating', 'prettify', 'ud.api', 'ud.api.v2', 'ud.extras', 'ud.form',
    'ud.lectureangular', 'ud.notetaking', 'ud.popup', "ud.initializer", 'ud.angular-loader'
        ], function($, Handlebars, handlebarsTemplates, constants, Raven) {
        'use strict';
    $.widget('ud.ud_dashboard', {
        ...
        _initHandlebarsTemplates: function() {
            ...
            this.genericErrorTemplate = Handlebars.compile($('#genericErrorTemplate').html());
        },
        ...
        prepareQuizSuccessHandler: function(quizId, quizzesXHR, completedQuizzesXHR) {
            ...
            if(quizzesXHR[0].status < 400 && completedQuizzesXHR[0].status < 400) {
                ...
                if(quiz.is_published || this.options.isInstructor) {
                    var template = (constants.quiz_type.CODING_EXERCISE == quiz.type) ?
            ...
            } else if(quizzesXHR[0].status < 500 && completedQuizzesXHR[0].status < 500) {
                Raven.captureMessage('Quiz is locked for user', {
                    'extra': {
                        'quizId': quizId,
                        'quizzesStatusCode': quizzesXHR[0].status,
                        'completedQuizzesStatusCode': completedQuizzesXHR[0].status
                    }
                });
